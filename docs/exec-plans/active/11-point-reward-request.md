...
- 10번 작업에서 `correct`와 `nextStep: 'point_reward'`로 분기된 경우에만 진입한다.
...
- 최신 `dev` 기준에서 `codex/11-point-reward-request` 형식의 작업 브랜치를 만든다.
- 의미 있는 기능 단위마다 커밋한다.
...
- 13번 작업은 `granted` 또는 `already_granted` 상태를 이어받아 오늘 학습 완료 화면을 보여준다.
- 14번 작업은 같은 `quizDate`에서 지급 요청이 반복되지 않도록 중복 보상 방지 가드를 정리한다.
- 10번 작업은 정답 결과와 `submissionId`, `promotionAmount`를 이 작업으로 전달한다.
