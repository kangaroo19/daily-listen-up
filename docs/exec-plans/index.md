...
- `completed` 문서에는 해당 `active` 문서의 완료 기준을 체크리스트 형태로 포함한다.
- 각 체크 항목에는 실제 확인 결과, 관련 파일, 실행한 검증 명령을 가능한 범위에서 함께 기록한다.
- 완료 기준을 충족하지 못한 항목이 있으면 미충족 사유와 후속 조치가 필요한 작업 번호를 기록한다.
...
- PR 생성은 `@github` 플러그인을 우선 사용한다.
  - 먼저 GitHub 플러그인의 PR 생성 도구를 검색하고 사용한다.
  - 로컬 `git`은 브랜치 생성, 커밋, 푸시에 사용하고, PR 생성은 GitHub 플러그인으로 처리한다.
  - GitHub 플러그인에서 PR 생성 도구를 찾지 못한 경우에만 사유를 기록하고 대안을 사용한다.
