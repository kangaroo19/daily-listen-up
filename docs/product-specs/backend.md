...
- 서버는 토스 사용자 정보에서 `userKey`를 확보한다.
- 프론트에는 토스 access token이나 원본 `userKey`를 내려주지 않는다.
- 프론트에는 `daily-listen-up` 앱 세션 토큰만 발급한다.
- 앱 세션 토큰은 KST 기준 당일 만료로 둔다.
- 앱 세션 토큰 만료는 토스 로그인 연결 자체를 끊는 정책이 아니다.

## 공식 문서 기준

- `appLogin()`은 클라이언트에서 인가 코드만 받는 역할을 한다.
- 토큰 교환, access token 발급, 사용자 정보 조회는 서버에서 처리한다.
- 비게임 프로모션은 서버 API 지급 방식을 사용한다.
- 포인트 하루 1회 지급 제어는 우리 서버가 담당한다.
- 리워드형 광고 보상은 `userEarnedReward` 이벤트 이후에만 서버에 기록한다.
...
- `GET /api/today-quiz`
  - 서버 KST 기준으로 오늘 문제, 완료 여부, 문제 없음 상태를 반환한다.
- `POST /api/answer-result`
  - 결과 확인 광고 완료 후 선택 답안을 검증하고 정답/오답 및 포인트 지급 상태를 반환한다.
- `GET /api/reward-status`
  - 정답 후 포인트 지급 상태를 `pending`, `success`, `failed` 중 하나로 반환한다.
- `POST /api/rewarded-ad-complete`
  - 리워드형 광고 완료 후 `retry` 또는 `script` 권한을 기록한다.
