...
const DEFAULT_BANNER_AD_GROUP_ID = 'ait-ad-test-banner-id';
...
export function getTossBannerAdGroupId(): string {
  return clientEnv.tossBannerAdGroupId || DEFAULT_BANNER_AD_GROUP_ID;
}
...
export async function attachTossBannerAd(target: HTMLElement): Promise<TossAdsAttachBannerResult | null> {
  ...
  try {
    return TossAds.attachBanner(getTossBannerAdGroupId(), target, getTossBannerOptions());
  } catch {
    return null;
  }
}
