...
관리자 앱은 운영 확인을 위해 유저 목록 조회 화면을 제공할 수 있다.

유저 목록 조회는 Firestore의 아래 컬렉션을 대상으로 한다.

```text
users
```

유저 목록 화면은 관리자 앱 사이드바의 `유저 관리` 화면으로 둘 수 있다.
목록은 최근 로그인 시각인 `loggedInAt` 기준 내림차순으로 페이지네이션해 조회한다.
운영자가 특정 사용자를 확인할 수 있도록 `userId` 단건 검색을 제공한다.

목록과 검색 결과에 표시하는 필드는 아래로 제한한다.

- `userId`
- 마스킹된 `userKey`
- `loggedInAt`

`userKey`는 토스 사용자 식별값이므로 원본 전체를 표시하지 않는다.
앞뒤 일부만 남기고 가운데를 마스킹하거나, 값이 짧으면 고정 마스킹 문구로 표시한다.

관리자 v1의 유저 목록 조회는 읽기 전용 운영 확인 기능이다.
유저 수정, 유저 삭제, 유저별 진행 기록 상세 조회, 유저별 보상 내역 상세 조회는 포함하지 않는다.
...
- Firestore Rules는 관리자 UID만 `users` 컬렉션을 읽을 수 있게 제한하며, 관리자 앱에서 `users` 컬렉션 쓰기는 허용하지 않는다.
...
- 유저별 진행 기록 상세 조회와 보상 내역 상세 조회
- 유저 수정, 삭제 같은 유저 관리 액션
- 유저 목록 조회를 위한 집계 또는 요약 컬렉션
